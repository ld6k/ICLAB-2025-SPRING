...
GATED_OR GATED_img_0 (.CLOCK(clk), .SLEEP_CTRL(cg_en && img_cg_en[0]), .RST_N(rst_n), .CLOCK_GATED(img_clk[0]));
GATED_OR GATED_img_1 (.CLOCK(clk), .SLEEP_CTRL(cg_en && img_cg_en[1]), .RST_N(rst_n), .CLOCK_GATED(img_clk[1]));
GATED_OR GATED_img_2 (.CLOCK(clk), .SLEEP_CTRL(cg_en && img_cg_en[2]), .RST_N(rst_n), .CLOCK_GATED(img_clk[2]));
GATED_OR GATED_img_3 (.CLOCK(clk), .SLEEP_CTRL(cg_en && img_cg_en[3]), .RST_N(rst_n), .CLOCK_GATED(img_clk[3]));
GATED_OR GATED_img_4 (.CLOCK(clk), .SLEEP_CTRL(cg_en && img_cg_en[4]), .RST_N(rst_n), .CLOCK_GATED(img_clk[4]));
GATED_OR GATED_img_5 (.CLOCK(clk), .SLEEP_CTRL(cg_en && img_cg_en[5]), .RST_N(rst_n), .CLOCK_GATED(img_clk[5]));
...
GATED_OR GATED_kernel_6 (
	.CLOCK(clk),
	.SLEEP_CTRL(cg_en && cond_kernel_clk_6),
	.RST_N(rst_n),
	.CLOCK_GATED(fuck_kernel_clk_6)
);

always @(posedge fuck_kernel_clk_6) begin
...
wire fuck_kernel_clk_7;
wire cond_kernel_clk_7 = !(!doin_0_or_1 && input_img_row == 7);
GATED_OR GATED_kernel_7 (
	.CLOCK(clk),
	.SLEEP_CTRL(cg_en && cond_kernel_clk_7),
	.RST_N(rst_n),
	.CLOCK_GATED(fuck_kernel_clk_7)
);

always @(posedge fuck_kernel_clk_7) begin
	if (!doin_0_or_1) begin
...
wire fuck_weight_clk_0;
wire cond_weight_clk_0 = !(!doin_0_or_1 && input_img_row == 6 && input_img_col[1] == 0);
GATED_OR GATED_weight_0 (
	.CLOCK(clk),
	.SLEEP_CTRL(cg_en && cond_weight_clk_0),
	.RST_N(rst_n),
	.CLOCK_GATED(fuck_weight_clk_0)
);

wire fuck_weight_clk_1;
wire cond_weight_clk_1 = !(!doin_0_or_1 && input_img_row == 6 && input_img_col[1] == 1);
GATED_OR GATED_weight_1 (
	.CLOCK(clk),
	.SLEEP_CTRL(cg_en && cond_weight_clk_1),
	.RST_N(rst_n),
	.CLOCK_GATED(fuck_weight_clk_1)
);

always @(posedge fuck_weight_clk_0) begin
...
always @(posedge fuck_weight_clk_1) begin
	if (!doin_0_or_1 && input_img_row == 6) begin
		if (input_img_col == 2) begin
...
always @(*) begin
	if (!conv_valid && pre_valid) begin
		input_conv_img[0][0] = 8'h0;
		input_conv_img[0][1] = 8'h0;
		input_conv_img[0][2] = 8'h0;

		input_conv_img[1][0] = 8'h0;
		input_conv_img[1][1] = 8'h0;
		input_conv_img[1][2] = 8'h0;

		input_conv_img[2][0] = 8'h0;
		input_conv_img[2][1] = 8'h0;
		input_conv_img[2][2] = 8'h0;
	end
	else if (input_img_row[1:0] == 1) begin
...
always @(*) begin
	if (!conv_valid && pre_valid) begin
		ker_reg_mux[0][0] = 8'h0;
		ker_reg_mux[0][1] = 8'h0;
		ker_reg_mux[0][2] = 8'h0;

		ker_reg_mux[1][0] = 8'h0;
		ker_reg_mux[1][1] = 8'h0;
		ker_reg_mux[1][2] = 8'h0;

		ker_reg_mux[2][0] = 8'h0;
		ker_reg_mux[2][1] = 8'h0;
		ker_reg_mux[2][2] = 8'h0;
	end
	else begin
...
wire fuck_conv_clk;
wire cond_conv_clk = cg_en && !(conv_out_valid || out_valid);
GATED_OR GATED_conv (
	.CLOCK(clk),
	.SLEEP_CTRL(cond_conv_clk),
	.RST_N(rst_n),
	.CLOCK_GATED(fuck_conv_clk)
);

always @(posedge fuck_conv_clk) begin
	if (out_valid) begin
		conv_out_reg <= 0;
	end
	else begin
...
// always @(*) begin
// 	fc_res[0] = max_pool[0] * weight_reg[0][0] + max_pool[1] * weight_reg[1][0];
// 	fc_res[1] = max_pool[0] * weight_reg[0][1] + max_pool[1] * weight_reg[1][1];
// end
...
wire fuck_encode_clk;
wire cond_encode_clk = !(encode_valid | (!delay_in_val1 && delay_in_val));
GATED_OR GATED_encode (
	.CLOCK(clk),
	.SLEEP_CTRL(cg_en && cond_encode_clk),
	.RST_N(rst_n),
	.CLOCK_GATED(fuck_encode_clk)
);

always @(posedge fuck_encode_clk) begin
		if (encode_valid) begin
...
// always @(*) begin
// 	out_data = (ker[0][0] * img[0][0]) + (ker[0][1] * img[0][1]) + (ker[0][2] * img[0][2]) +
// 	           (ker[1][0] * img[1][0]) + (ker[1][1] * img[1][1]) + (ker[1][2] * img[1][2]) +
// 	           (ker[2][0] * img[2][0]) + (ker[2][1] * img[2][1]) + (ker[2][2] * img[2][2]);
// end
...
// module fc(
// 	input  [7:0] max_pool [0:1],
// 	input  [7:0] weight_reg [0:1][0:1],
// 	output reg  [7:0] fc_res_quant [0:1]
// );
// reg [16:0] fc_res [0:1];
